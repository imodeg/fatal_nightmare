...
(function (PhaserSpine) {
    ...
    (function (Canvas) {
        var Renderer = (function () {
            ...
            Renderer.prototype.findAltitudeNormal = function () {
            };
            Renderer.prototype.drawTriangle = function (renderSession, img, x0, y0, u0, v0, x1, y1, u1, v1, x2, y2, u2, v2) {
                ...
                u2 *= img.width;
                v2 *= img.height;
                var xCenter = (x0 + x1 + x2) / 3;
                var yCenter = (y0 + y1 + y2) / 3;
                var VL;
                var DevideFactor = 1;
                var xV0 = x0 - xCenter;
                var yV0 = y0 - yCenter;
                VL = Math.sqrt((xV0 * xV0) + (yV0 * yV0));
                xV0 = (xV0 / VL) / DevideFactor;
                yV0 = (yV0 / VL) / DevideFactor;
                var xV1 = x1 - xCenter;
                var yV1 = y1 - yCenter;
                VL = Math.sqrt((xV1 * xV1) + (yV1 * yV1));
                xV1 = (xV1 / VL) / DevideFactor;
                yV1 = (yV1 / VL) / DevideFactor;
                var xV2 = x2 - xCenter;
                var yV2 = y2 - yCenter;
                VL = Math.sqrt((xV2 * xV2) + (yV2 * yV2));
                xV2 = (xV2 / VL) / DevideFactor;
                yV2 = (yV2 / VL) / DevideFactor;
                ctx.beginPath();
                ctx.moveTo(x0 + xV0, y0 + yV0);
                ctx.lineTo(x1 + xV1, y1 + yV1);
                ctx.lineTo(x2 + xV2, y2 + yV2);
                ctx.closePath();
                x1 -= x0;
                y1 -= y0;
                x2 -= x0;
...
(function (PhaserSpine) {
    var Spine = (function (_super) {
        ...
        function Spine(game, x, y, key) {
            ...
            var anhorX = (_this.getBounds().width / 2 / _this.scale.x) / _this.getBounds().width;
            var anhorY = (_this.getBounds().height / _this.scale.y) / _this.getBounds().height;
            _this.anchor.setTo(anhorX, anhorY);
        ...
        Spine.prototype.update = function () {
            ...
            if (this.state.tracks[0].isComplete()) {
